...
/* LISTE STATIQUE DES DEVISES — plus d'appel API */
...
export function TicketForm({ threadId, onClose }: Props) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
             style={{ background: 'rgba(7, 13, 9, 0.75)', backdropFilter: 'blur(8px)' }}>
            ...
            <div className="w-full max-w-lg rounded-2xl overflow-hidden flex flex-col max-h-[92vh]"
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b flex-shrink-0"
                    ...
                    <div>
                        <h2 className="text-base font-bold text-white">Nouveau ticket</h2>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Field label={`Montant ${selectedCurrency?.code ?? ''}`}>
                            <NumberInput value={amount} onChange={setAmount} placeholder="5000" step="1" min={0} />
                        </Field>
                        <Field label="Taux négocié (TND)">
                            <NumberInput value={price} onChange={setPrice} placeholder="3.3790" step="0.0001" min={0} />
                        </Field>
                    </div>
                    ...
                    {amountNum > 0 && parseFloat(price) > 0 && (
                        <div className="px-3 py-2 rounded-lg text-center"
                            ...
                            <div className="font-mono-nums text-lg font-bold text-white mt-0.5">
                                {tndEquivalent} <span className="text-xs" style={{ color: '#a8c4aa' }}>TND</span>
                            </div>
                        </div>
                    )}
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    <div className="text-[11px] font-semibold text-white">
                                        ...
                                        <span className="ml-2 text-[9px] font-normal" style={{ color: '#5a8060' }}>
                      (optionnel)
                    </span>
                                    </div>
                                    <div className="text-[9px]" style={{ color: '#5a8060' }}>
                                        Si ajoutées, le total doit être égal au montant
                                    </div>
                                </div>
                                <button
                                    type="button"
                                    ...
                                    className="px-2 py-1 rounded-lg text-[10px] flex items-center gap-1 transition-all disabled:opacity-30 hover:scale-105"
                                    style={{
                            ...
                            {bills.length === 0 && (
                                <div className="text-center py-2 text-[10px]" style={{ color: '#5a8060' }}>
                                    Aucune coupure ajoutée (facultatif)
                                </div>
                            )}
                                ...
                                <div key={i} className="flex items-center gap-2">
                                    ...
                                    <div className="flex items-center gap-1">
                                        ...
                                        <input
                                            ...
                                            className="w-14 text-center px-2 py-1 rounded-lg text-sm font-mono-nums focus:outline-none"
                                            style={{
                            ...
                            {bills.length > 0 && (
                                <div className="flex items-center justify-between pt-2 border-t"
                                    ...
                                    <span className="text-[10px]" style={{ color: '#a8c4aa' }}>Total coupures</span>
                    ...
                    <Field label="Validité du ticket">
                        ...
                        <div className="mt-2 flex items-center gap-2">
                            <span className="text-[10px]" style={{ color: '#5a8060' }}>ou minutes :</span>
                            <div className="w-36">
                                <NumberInput
            ...
            />
                ...
                <div className="flex gap-3 p-4 border-t flex-shrink-0"
                    ...
                    <Button className="flex-1" loading={create.isPending}
...
function Select({
    ...
    return (
        <select value={value} onChange={(e) => onChange(e.target.value)}
                className={`w-full px-3 py-2 text-sm rounded-xl focus:outline-none transition-colors cursor-pointer ${className ?? ''}`}
                style={{
    ...
    return (
        <div className="flex items-center gap-1">
            <button type="button"
                    ...
                    className="w-8 h-9 rounded-lg flex items-center justify-center transition-all active:scale-90 flex-shrink-0"
                    style={{
                        background: 'rgba(42, 128, 64, 0.15)',
                        border: '1px solid rgba(74, 222, 128, 0.25)',
                        color: '#4ade80',
            ...
            <input type="number" step={step} value={value}
                   ...
                   className="flex-1 min-w-0 px-3 py-2 text-sm text-center font-mono-nums rounded-xl focus:outline-none transition-colors"
                   style={{
                       background: 'rgba(0, 0, 0, 0.3)',
                       border: '1px solid rgba(255,255,255,0.08)',
                       color: '#fff',
