...
function formatDeliveryMode(mode: string): string {
  return { CASH:'Espèces', ACCOUNT:'Virement compte', TRANSFER:'Virement externe', CHEQUE:'Chèque', BORDEREAU:'Bordereau' }[mode] ?? mode
}
...
export default function AgencePage() {
  ...
  const search = useMutation({
    mutationFn: async (ref: string) => (await api.get<TicketDetails>(`/branch/orders/${ref}`)).data,
    onSuccess: (data) => { setTicket(data); setError(null); setNote('') },
    onError: (err: any) => {
      setTicket(null)
  ...
  const processMutation = useMutation({
    mutationFn: async () => (await api.post(`/branch/orders/${ticket!.ref_ticket}/confirm-processing`, {})).data,
    onSuccess: () => { toast.success('Ticket pris en charge'); search.mutate(ticket!.ref_ticket) },
    onError: (err: any) => {
      const detail = err.response?.data?.detail
  ...
  const completeMutation = useMutation({
    mutationFn: async () => (await api.post(`/branch/orders/${ticket!.ref_ticket}/confirm-completed`, { note: note || null })).data,
    onSuccess: () => { toast.success('Ticket finalisé avec succès'); setNote(''); search.mutate(ticket!.ref_ticket) },
    onError: (err: any) => {
      const detail = err.response?.data?.detail
  ...
  const handleSearch = () => {
    ...
    if (!trimmed) return
    search.mutate(`FX-${trimmed.padStart(6, '0')}`)
  }
  ...
  const clientDisplay = ticket ? ticket.client_name ?? ticket.client_username ?? `Client #${ticket.client_id}` : ''
  ...
  return (
      <div className="min-h-screen flex flex-col" style={{ background: '#070d09' }}>
        ...
        <header
            ...
            style={{ background: 'rgba(15, 58, 26, 0.7)', borderColor: 'rgba(42, 128, 64, 0.3)' }}
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-xs sm:text-sm font-semibold text-white">Espace Agence</h1>
          ...
          <button
              ...
              style={{ background: 'rgba(15, 58, 26, 0.4)', border: '1px solid rgba(42, 128, 64, 0.3)', color: '#a8c4aa' }}
              onMouseEnter={(e) => {
        ...
        <main className="flex-1 p-4 sm:p-6 max-w-3xl mx-auto w-full overflow-y-auto">

          {/* RECHERCHE */}
          <div
              className="p-4 sm:p-5 rounded-2xl mb-4 sm:mb-6"
              style={{
            ...
            <div className="flex gap-2">
              ...
              >
                <div
                    ...
                    style={{ background: 'rgba(74, 222, 128, 0.1)', borderRight: '1px solid rgba(74, 222, 128, 0.2)' }}
                >
                  <span className="font-mono-nums text-sm font-bold" style={{ color: '#4ade80' }}>FX-</span>
                </div>
                <input
                    value={refNumber}
                    onChange={(e) => setRefNumber(e.target.value.replace(/\D/g, '').slice(0, 6))}
                    onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
                    placeholder="000084"
                    inputMode="numeric"
                    maxLength={6}
                    className="flex-1 px-3 py-3 text-sm bg-transparent font-mono-nums focus:outline-none tracking-wider"
              ...
              <Button
                  ...
                  className="px-4 sm:px-5 min-h-[48px]"
              >
                Rechercher
              </Button>
            </div>
          ...
          {error && (
              <div
                  className="flex items-center gap-3 p-4 rounded-xl mb-4 sm:mb-6"
                  style={{ background: 'rgba(200, 16, 46, 0.1)', border: '1px solid rgba(251, 113, 133, 0.3)' }}
              >
          ...
          {ticket && status && (
              ...
              >
                ...
                <div
                    ...
                    style={{ background: 'rgba(26, 92, 42, 0.2)', borderColor: 'rgba(42, 128, 64, 0.25)' }}
                >
                  <div>
                    <div className="text-[10px] uppercase tracking-widest" style={{ color: '#a8c4aa' }}>Référence</div>
                    <div className="font-mono-nums text-base font-bold text-white mt-0.5">{ticket.ref_ticket}</div>
                  </div>
                  <div
                ...
                <div className="p-4 sm:p-5 space-y-4">
                  ...
                  <div
                      ...
                      style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(42, 128, 64, 0.25)' }}
                  >
                    <div className="text-[10px] uppercase tracking-widest mb-1" style={{ color: '#5a8060' }}>Opération</div>
                  ...
                  {/* Grille infos — 1 col sur très petit mobile, 2 col sinon */}
                  ...
                  {ticket.negotiated_price != null && (
                      ...
                      >
                        <div className="text-[10px] uppercase tracking-widest mb-1" style={{ color: '#a8c4aa' }}>Taux négocié</div>
                  ...
                  {ticket.trader_comment && (
                      <div className="px-3 py-2 rounded-lg italic text-xs"
                           style={{ background: 'rgba(26, 92, 42, 0.2)', borderLeft: '2px solid #2a8040', color: '#e8f0e9' }}>
                        <div className="text-[9px] uppercase tracking-wider mb-1 not-italic" style={{ color: '#5a8060' }}>Commentaire trader</div>
                        « {ticket.trader_comment} »
                      </div>
                  )}
                  ...
                  {ticket.branch_comment && (
                      <div className="px-3 py-2 rounded-lg italic text-xs"
                           style={{ background: 'rgba(96, 165, 250, 0.1)', borderLeft: '2px solid #60a5fa', color: '#e8f0e9' }}>
                        <div className="text-[9px] uppercase tracking-wider mb-1 not-italic" style={{ color: '#60a5fa' }}>Commentaire agence</div>
                        « {ticket.branch_comment} »
                      </div>
                  )}
                  ...
                  {ticket.bills && ticket.bills.length > 0 && (
                      <div className="px-3 py-2.5 rounded-lg space-y-2"
                           style={{ background: 'rgba(0, 0, 0, 0.25)', border: '1px solid rgba(42, 128, 64, 0.25)' }}>
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                          {ticket.bills.map((b) => (
                              <div key={b.id}
                                   ...
                                   style={{ background: 'rgba(42, 128, 64, 0.12)', border: '1px solid rgba(74, 222, 128, 0.15)' }}>
                  ...
                  {canComplete && (
                      <div className="space-y-2">
                        <label className="text-[10px] uppercase tracking-wider flex items-center gap-1.5" style={{ color: '#a8c4aa' }}>
                        ...
                        />
                        <p className="text-[10px]" style={{ color: '#5a8060' }}>Ce commentaire sera enregistré avec le ticket finalisé.</p>
                      </div>
                  )}

                  {/* Actions */}
                  {canProcess && (
                      <ActionButton loading={processMutation.isPending} onClick={() => processMutation.mutate()}
                                    icon={<ClipboardCheck size={16} />} label="Prendre en charge" />
                  )}

                  {canComplete && (
                      <ActionButton loading={completeMutation.isPending} onClick={() => completeMutation.mutate()}
                                    icon={<CheckCircle2 size={16} />} label="Finaliser le ticket" />
                  )}

                  {/* Workflow timestamps */}
                  {(ticket.branch_confirmed_at || ticket.completed_at) && (
                      <div className="px-3 py-2 rounded-lg space-y-1.5"
                           style={{ background: 'rgba(0, 0, 0, 0.25)', border: '1px solid rgba(42, 128, 64, 0.2)' }}>
                        <div className="text-[10px] uppercase tracking-wider mb-1" style={{ color: '#a8c4aa' }}>Workflow</div>
          ...
          {!ticket && !error && !search.isPending && (
              <div className="text-center py-12 sm:py-16">
                ...
                <p className="text-sm font-medium" style={{ color: '#a8c4aa' }}>Recherchez un ticket</p>
...
function InfoRow({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) {
  return (
      <div className="px-3 py-2 rounded-lg" style={{ background: 'rgba(0, 0, 0, 0.25)', border: '1px solid rgba(42, 128, 64, 0.2)' }}>
        <div className="flex items-center gap-1 mb-1">
          {icon && <span style={{ color: '#5a8060' }}>{icon}</span>}
          <span className="text-[9px] uppercase tracking-wider" style={{ color: '#5a8060' }}>{label}</span>
        </div>
        <div className="font-mono-nums text-sm font-semibold text-white truncate">{value}</div>
      </div>
  )
}

function ActionButton({ loading, onClick, icon, label }: { loading: boolean; onClick: () => void; icon: React.ReactNode; label: string }) {
  return (
      <button
          onClick={onClick}
          disabled={loading}
          className="w-full py-3 sm:py-3 rounded-xl font-semibold text-sm
                     flex items-center justify-center gap-2 text-white
                     transition-all active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed min-h-[48px]"
          style={{
